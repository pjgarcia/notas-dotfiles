...
;; listas las interfaces de las term-list
;; ahora queda hacer que el paquete de polys
;; las use
...
(define (install-polynomial-package2)
  ...
  (define (mul-term-by-all-terms t1 L)
	...
	(the-empty-sparse-termlist)
	(let ((t2 (first-term L)))
	  (adjoin-term (make-sparse-term (+ (order t1) (order t2))
					 (mul (coeff t1) (coeff t2)))
		       (mul-term-by-all-terms t1 (rest-terms L))))))
  (define (add-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
  ...
  (define (negative-terms terms)
	...
	(the-empty-sparse-termlist)
	(let ((first (first-term terms)))
	  (adjoin-term (make-sparse-term (order first)
					 (negative (coeff first)))
		       (negative-terms (rest-terms terms))))))
  (define (negative-poly p)
    (make-poly
